...
% save eps file
%
% modified by Olivier Crouzet
% Dpt of Communication and Neuroscience, Keele University, UK
% o.crouzet@cns.keele.ac.uk
% '-deps' --> '-depsc'
if ~loose
  cmd=[ 'print(''-depsc'',''-f' int2str(figno) ''',''' epsfullnameext ''')' ];
else
  cmd=[ 'print(''-depsc'',''-loose'',''-f' int2str(figno) ...
	''',''' epsfullnameext ''')' ];
end  
...
%  fprintf(fid,[ '\\parbox{' num2str(latexwidth) 'cm}{\\centering%%\n' ]);
  fprintf(fid,[ '\\parbox{\\linewidth}{\\centering%%\n' ]);
end  
if noscalefonts
