function [y,Fs,wavload_filename,bits,opt_ck] = wavload(file,ext)
% Read Microsoft WAVE (".wav") sound file (Modified, type help for more info).
%
% Modifications (Olivier Crouzet):
% waveload_filename: GLOBAL variable accessible to other functions.
% filename may contain a '.', wavload will add a '.wav' to try opening the file.
%
%   Y=wavload(FILE) reads a WAVE file specified by the string FILE,
...
%
%   [Y,FS,NBITS]=wavload(FILE) returns the sample rate (FS) in Hertz
%   and the number of bits per sample (NBITS) used to encode the
%   data in the file.
%
%   [...]=wavload(FILE,N) returns only the first N samples from each
%       channel in the file.
%   [...]=wavload(FILE,[N1 N2]) returns only samples N1 through N2 from
%       each channel in the file.
%   SIZ=wavload(FILE,'size') returns the size of the audio data contained
%       in the file in place of the actual audio data, returning the
%       vector SIZ=[samples channels].
%
%   [Y,FS,NBITS,OPTS]=wavload(...) returns a structure OPTS of additional
