...
%
% Procédure adaptative à contraintes fortes : PEST (Bonnet, 19?? ; Levitt & Rabiner, 1967)
%
% USAGE : pest;
...
% reads the names of the signals to play
		signal_file = deblank(signal_list(signals_order(n)).file);
		signal_path = [root_disk,root_signal_dir,signal_file,'.wav'];
		[signal,Fs] = wavread(signal_path);
		answer = deblank(answer_list(signals_order(n)).file);

		noise_file = deblank(noise_list(noises_order(n)).file);
		noise_path = [root_disk,root_noise_dir,noise_file,'_noise.wav'];
		[noise,Fs] = wavread(noise_path);
...
%		if strcmp(DATA_VALUE_STORED,answer) == 1, % l(10): line ID, .file(1:3): first three characters
%				hits(n) = 1;
%			else
%				hits(n) = 0;
%			end

%		t_hits(1,n) = sum(hits(1:n),2);

%		Ecrit les résultats dans un fichier
		fprintf(data_file_ID,'%s\t%6.0f\t%s\t%s\t%6.0f\t%6.6f\t%s\t%s\t%6.0f\t%6.0f\t%6.3f\t%6.3f\n',...
			subjectID, n, signal_file, noise_file, inversion(n),snr(n),DATA_VALUE_STORED,answer,hits(n),sum(hits(1,1:n),2),(p_hits_threshold*(n-1))-constant,(p_hits_threshold*(n-1))+constant);
