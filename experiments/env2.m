...
root_disk = 'c:\';
root_signals_dir = 'home\olivier\desync\additive\stimuli\';
root_expe = 'c:\home\olivier\desync\additive\';
cd(root_expe);
%root_disk = 'd:\';
...
%	break;
	...
	noise_listID=[listID,'_noises.txt'];
	noisephase_listID=[listID,'_noisesphase.txt'];
	
	signal_list = readlist(signal_listID);
	nb_stim = size(signal_list);
	nb_stim = nb_stim(:,2);

	noise_list = readlist(noise_listID);
	nb_noise = size(noise_list);
	nb_noise = nb_noise(:,2);
	
	phase_list = readlist(noisephase_listID);
	nb_phase = size(phase_list);
	nb_phase = nb_phase(:,2);
...
% reads the names of the signals to play
			signal_file = deblank(signal_list(signals_order(n)).file);
			signal_path = [root_disk,root_signals_dir,signal_file,'_',signal_extension,'.wav']
...
%random selection of the noise file
			rand('state',sum(100*clock));
			noise_order = randperm(nb_noise);
			noise_file = deblank(noise_list(noise_order(1)).file);
         phase = str2num(phase_list(noise_order(1)).file);
         noise_path = [root_disk,root_signals_dir,'noise\','noise_',noise_extension,'_',noise_file]
			[noise,Fs] = wavload(noise_path);
         
% Shortening the noise signal to the size of the signal
			noise = zeroends(noise(1:length(signal)),Fs);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% DIGIT RECOGNITION PERFORMANCE
			hits(1,n) = performance(DATA_VALUE_STORED,answer);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% STRING RECOGNITION PERFORMANCE
%			hits(1,n) = strcmp(DATA_VALUE_STORED,answer);
			fprintf(data_file_ID,'%s\t%6.0f\t%s\t%s\t%s\t%s\t%6.6f\t%6.0f\t%6.6f\t%s\t%s\t%6.0f\t%6.0f\t%6.3f\t%6.3f\n',...
				subjectID, n, signal_extension, signal_file, noise_extension, noise_file, phase, inversion(n),snr(n),DATA_VALUE_STORED,answer,hits(n),sum(hits(1,1:n),2),(p_hits_threshold*(n-1))-constant,(p_hits_threshold*(n-1))+constant);
